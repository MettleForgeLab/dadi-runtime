...
def test_verify_returns_closure_or_error():
    token = make_jwt(JWT_SECRET, {"tenant_id":"tenant_a","scope":"artifact:read_bytes","exp": int(time.time()) + 3600})
    headers = {"Authorization": f"Bearer {token}"}
    # nonexistent manifest should 404/400/403; ensure not 500
    r = requests.post(
        f"{API}/deliverables/00000000-0000-0000-0000-000000000000/bundle/verify",
        json={"manifest_artifact_sha256": "0"*64},
        headers=headers,
    )
