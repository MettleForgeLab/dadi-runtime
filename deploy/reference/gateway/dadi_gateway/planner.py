...
def create_plan(req: RegenerateRequest) -> Dict[str, Any]:
    ...
    with tx() as c:
        c.execute(
            ...
            (plan_id, req.model_dump_json(), json.dumps(plan_json)),
        )
...
def get_plan(plan_id: str) -> Dict[str, Any]:
    with conn() as c:
        row = c.execute("SELECT status, plan_json FROM regeneration_plans WHERE plan_id=%s", (plan_id,)).fetchone()
    if not row:
...
def explain_plan(plan_id: str) -> Dict[str, Any]:
...
def mark_executed(plan_id: str) -> None:
    with tx() as c:
        c.execute("UPDATE regeneration_plans SET status='executed' WHERE plan_id=%s", (plan_id,))
