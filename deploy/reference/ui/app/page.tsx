    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Card>
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
            <Field label="Artifact SHA256">
              <input
                ...
                style={{ width: 520, padding: "10px 12px", borderRadius: 10, border: "1px solid #e5e7eb" }}
              />
            </Field>
            <button
          ...
          {meta && (
            <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Field label="artifact_type"><code>{meta.artifact_type}</code></Field>
              <Field label="schema_version"><code>{meta.schema_version || "(none)"}</code></Field>
              <Field label="media_type"><code>{meta.media_type}</code></Field>
              <Field label="byte_length"><code>{meta.byte_length}</code></Field>
              <Field label="canonical"><code>{String(meta.canonical)}</code></Field>
              <Field label="canonical_format"><code>{meta.canonical_format || "(none)"}</code></Field>
              <Field label="storage_backend"><code>{meta.storage_backend}</code></Field>
              <Field label="storage_ref"><code>{meta.storage_ref || "(none)"}</code></Field>
            </div>
          )}

          {contentJson ? (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>Content (JSON)</div>
              <JsonView value={contentJson} />
            </div>
          ) : contentText ? (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>Content (text preview)</div>
              <pre
                style={{
                  margin: 0,
                  padding: 12,
          ...
          {(upstream || downstream) && (
            <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>Upstream edges</div>
                <JsonView value={upstream || []} />
              </div>
              <div>
                <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>Downstream edges</div>
      ...
      {tab === "plan" && (
        <Card>
          ...
          {planExplain && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>Explain</div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
            <Field label="Artifact A SHA256">
              <input
                ...
                placeholder="64-hex SHA256"
                style={{ width: 460, padding: "10px 12px", borderRadius: 10, border: "1px solid #e5e7eb" }}
              />
            </Field>
